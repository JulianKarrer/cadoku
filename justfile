...
build-web:
    ...
    echo "const version = \"$(date -u +"%Y-%m-%dT%H:%M:%SZ")\";" > public/cached_asset_list.js && echo "const offlineFundamentals = [" >> public/cached_asset_list.js && find assets -type f | sed 's|^|    \"cadoku/|; s|$|\",|' >> public/cached_asset_list.js && sed -i '$ s/,$//' public/cached_asset_list.js && echo "]" >> public/cached_asset_list.js
